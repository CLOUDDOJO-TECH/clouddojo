...
import { useGetQuizAttempts } from "../hooks/useQuizAttempt";
...
export function AttemptsDialog({ attemptId }: { attemptId: string }) {
  ...
  const { quizAttemptsData, quizAttemptLoading, quizAttemptError } =
    useGetQuizAttempts({ attemptId });
  ...
  const transformedAnswers = quizAttemptsData
    ? quizAttemptsData.questions.reduce(
        (acc: Record<string, string[]>, q: QuizAttemptQuestion) => {
          return {
            ...acc,
