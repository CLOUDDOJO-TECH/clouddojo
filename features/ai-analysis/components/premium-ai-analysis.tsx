...
export default function PremiumAnalysisDashboard() {
  ...
  useEffect(() => {
    const calculateTimeUntilNextUpdate = () => {
      ...
      const timeUntil = formatDistanceToNow(nextUTCMidnight, {
        addSuffix: true,
      });
    ...
    return (
      <div className="p-6 text-center h-[50vh] flex flex-col items-center justify-center">
        <div className="text-brand-beige-950 mb-4 flex items-center justify-center">
          <AlertCircle className="h-5 w-5 mr-2" /> An Error Occurred. Please try
          again later.
        </div>
  ...
  return (
    <div className="mx-auto px-3 py-6 bg-none" ref={reportRef}>
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="text-primary italic font-serif max-w-[200px] text-center">
                      <p>
                        AI analysis is refreshed daily at midnight (UTC) to
                        optimize resource usage and provide consistent insights.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
      ...
      {report.studyPlan && (
        <PersonalizedStudyPlan studyPlan={report.studyPlan} />
      )}
    </div>
  );
}
